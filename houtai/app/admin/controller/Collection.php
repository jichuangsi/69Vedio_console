 ...
 * @time 2019-12-17
 */
class Collection extends Admin
{
    ...
    function lists(Request $request){
        ...
        $data_list=$this->myDb->view('video','id,title,info,key_word,update_time,click,good,thumbnail,user_id,status,is_check,sort,type,gold,recommend')
            ...
            ->view('video_collection','collection_time,id as vcid','video.id=video_collection.video_id')
            ->view('member','username','video_collection.user_id=member.id')
            ->where($where)->order("collection_time",'desc')->cache(120)->paginate(null,false,['query'=>$request->get()]);
